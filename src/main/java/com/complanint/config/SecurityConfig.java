...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http)
        ...
        // 페이지 이동 하는데 로그인 해야 이용할 수 있는 경우 로그인 페이지로 이동한다.
        // 로그인 성공하면 이전에 방문 했던 페이지로 이동할 수 있는 기능을 제공한다.
        ...
        http.requestCache( rq -> rq.requestCache(requestCache)).
                authorizeHttpRequests(
                        ar -> ar
                                ...
                                .anyRequest().authenticated() // 나머지 모든 주소요청은 로그인해야만 한다.
                )
                .formLogin(
                        form -> form
                                ...
                                .failureUrl("/signIn/error")// 로그인실패시 어디로?
                                .permitAll()
                )
                .logout(
                        out -> out
                                .logoutRequestMatcher(new AntPathRequestMatcher("/logout","GET"))
                                .logoutSuccessUrl("/")//로그아웃 성공하면 어디로?
                                .invalidateHttpSession(true)//로그아웃되면 모든 세션 제거

                );
        
        // .logoutUrl("/logout") post 요청 일경우 실행됨
